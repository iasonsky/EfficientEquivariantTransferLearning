...
ALL_MODELS = sum(
	(
		tuple(conf_archiv_map.keys())
		for conf_archiv_map in (BERT_PRETRAINED_CONFIG_ARCHIVE_MAP, ROBERTA_PRETRAINED_CONFIG_ARCHIVE_MAP)
	),
	(),
)
			...
			if (step + 1) % args.gradient_accumulation_steps == 0:
				...
				if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
					...
					if (
						...
						results, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode=DEV_FILE_PATTERN)
...
def main():
	...
	parser.add_argument(
		"--data_dir",
		default="data/regard/occupation/gender/",
		type=str,
		required=False,
		help="The input data dir. Should contain the training files for the regard classification task.",
	)
	parser.add_argument(
		"--test_file",
		default="occupation_man_equi.txt",
		type=str,
		required=False,
	...
	parser.add_argument(
		"--output_dir",
		default="models/bert_regard_v2_large",
		type=str,
		required=False,
		help="The output directory where the checkpoints will be written (predictions are written at output_pred_dir).",
	)
	...
	# Training
	if args.do_train:
		train_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, data_file=TRAIN_FILE_PATTERN, is_test=False)
	...
	if args.do_eval and args.local_rank in [-1, 0]:
		...
		for checkpoint in checkpoints:
			...
			model.to(args.device)
			result, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode=DEV_FILE_PATTERN, prefix=global_step, is_test=False)
			if global_step:
	...
	if args.do_predict and args.local_rank in [-1, 0]:
		...
		# note: set is_test=False to check the performance of classifier. Since labels for general input is unknown,
		# we use is_test=True, which means the labels are randomly assigned as 0
